...
#inlcude():하위 URLConf파일을 추가할 때 사용하는 함수
# urlpatterns : URL과 뷰함수를 등록 및 관리하는 변수
#RUL등록 시 path 함수를 이용해 urlpatterns의 요소를 추가
#path(URL주소( 문자열),  호출할 뷰함수/클래스 이름)
#웹서버 기본주소:127.0.0.1:8000/
urlpatterns = [
    #기본주소+admin 주소로 클라이언트가 요청한 경우, admin 폴더/site.py/urls 함수가 실행
    path('a1/', admin.site.urls),
    path('',index), #127.0.0.1:8000으로 클라이언트가 요청했을 때, index함수가 호출
    path('list/',booklist),
    #127.0.0.1:8000/detail/숫자/로 요청시 bookdetail 호출, 숫자데이터는 bookid 저장이 됨
    ...
    #해당 어플리케이션의  app_name 변수에 저장된 문자열 대신 사용할 그룹이름을 namespace 매개변수에 대입시키면 됨
    #127.0.0.1:8000/auth/
    path('auth/', include('social_django.urls', namespace='social')),
    path('blog/', include('blog.urls'))
]
from django.conf import settings #setting.py에 설정된 값을 가져오기 위함
from django.conf.urls.static import static#Media_url과 root을 연동하기 위한 함수
