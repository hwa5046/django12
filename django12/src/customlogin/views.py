...
def signup(request):
    if request.method == "GET":
        ...
        if form.is_valid(): #아이디 중복체크, 아이디 형식체크, 비밀번호 형식체크
            #form.save() 는 비밀번호를 암호화하지 않은채로 원본데이터를 저장하기 때문에 사용하면 안됨
            #User.objects.create_user(아이디, 이메일, 비밀번호):비밀번호를 암화한채로 회원생성, 데이터베이스에 저장 및 반환
            #form.cleaned_data['변수명']->사용자가 입력한 데이터 추출
            
            #비밀번호와 비밀번호확인에 동일한 입력을 했는지 확인
            ...
            #새로운 회원 생성 및 데이터베이스에 저장
                ...
                return render(request, 'customlogin/signup.html',{'f':form, 'error':"비밀번호 확인이 다릅니다."})
        else:
...
def signin(request):
    if request.method=="GET":
        ...
        #아이디와 비밀번호가 일치하지 않는 경우 None값 반환, 일치하는 경우는  User객체를 반환
        u= authenticate(username=id, password=pw)
        #u 변수에 User객체가 저장됬는지 확인
        if u:#아이디와 비밀번호가 일치하는 User객체가 u 변수에 저장되어있는경우
            #해당요청을 한 클라이언트가 u에 저장된 User객체로 로그인
