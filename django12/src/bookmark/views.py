...
#함수 형태로 정의 시 헛번째 매개변수를 request로 정의해야함
#request: 사용자의 요청정보, <form>으로 넘겨준 데이터, 로그인정보, 세션정보, 요청방식(Get, Post)

#HTML 파일을 전송하는 메인페이지
def index(request):
    #render(request, HTML 문서의 경로, HTML문서에 전달할 값(사전형))
    return render(request, 'bookmark/index.html', {'a': 'hello', 'b':[1,2,3] }) 

#모델클래스에 저장된 객체들을 추출하기 위해서 모델 클래스 임포트
from .models import Bookmark
#from bookmark.models import Bookmark

#Bookmark 모델클래스의 객체를 추출해 HTML 파일을 수정하는 페이지
def booklist(request):
    #모델클래스명.objects.all(): 데이터베이스에 해당 모델클래스로 저장된 모든 객체를 리스트형태로 추출
    #모델클래스명.objects.get(): 데이터베이스에 해당 모델클래스로 저장된 객체 중 한개를 추출
