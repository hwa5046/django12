...
class Post(models.Model):
    #model.PROTECT : 연결된 객체가 삭제되는것을 막아주는 기능
    #model.SET_NULL:연결된 객체가 삭제되면 null값을 저장
    ...
    #blank(기본값 False):폼객체.is_valid(), 폼객체.as_p을 사용했을 때 사용자가 입력하지 않아도 되도록 허용
    content = models.TextField('내용', null= True, blank = True)
    #auto_now_add:객체가 생성될 때 자동으로 서버의 현재시간을 저장하도록 설정
...
class PostImage(models.Model):
    ...
    #%Y:해당 서버의 현재 년도, %m: 해당 서버의 현재 월, %d: 해당서버의 현재 일
    
    #객체 삭제 시 호출되는 함수, 객체 삭제 전 실제 이미지파일을 삭제하는 코드를 삽입
    def delete(self, using=None, keep_parents=False):
        #image변수에 저장되있는 경로의 파일을 지우는과정
        self.image.delete()
